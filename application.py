...
@app.route('/category/<categoryname>/menu/JSON')
...
@app.route('/category/<categoryname>/catalog_item/<int:id>/JSON')
...
@app.route('/category/')
def showCategory():
    category = session.query(Category).order_by(asc(Category.categoryname))
...
def newCategory():
    ...
    if request.method == 'POST':
        newCategory = Category(
            name=request.form['categoryname'], user_id=login_session['user_id'])
...
def editCategory(category_id):
    ...
    if editedCategory.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('Not authorized to edit');}</script><body onload='myFunction()'>"
    if request.method == 'POST':
...
def deleteCategory(category_id):
    ...
    if categoryToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('Not authorized to delete');}</script><body onload='myFunction()'>"
    if request.method == 'POST':
...
def showCatalog(catalog_id):
    catalog = session.query(CatalogItem).filter_by(id=catalog_id).one()
    creator = getUserInfo(catalog.user_id)
    items = session.query(CatalogItem).filter_by(
        category_id=category_id).all()
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('publiccatalog.html', items=items, catalog=catalog, creator=creator)
    else:
        return render_template('catalog.html', items=items, catalog=catalog, creator=creator)
...
def newCatalogItem(catalog_id):
    ...
    if login_session['user_id'] != catalog.user_id:
        return "<script>function myFunction() {alert('Not authorized, please get your own.');}</script><body onload='myFunction()'>"
        if request.method == 'POST':
